...
# Complexity:
# __init__ : T -> O(M * N); S -> O(M * N)
# sumRegion : T -> O(1); S -> O(1)
...
# Testing:
funcs = ["NumMatrix", "sumRegion", "sumRegion", "sumRegion"]
params = [[[[3, 0, 1, 4, 2], [5, 6, 3, 2, 1], [1, 2, 0, 1, 5], [4, 1, 0, 1, 7], [1, 0, 3, 0, 5]]], [2, 1, 4, 3], [1, 1, 2, 2], [1, 2, 2, 4]]
matrix = params[0][0]
   
instance = NumMatrix(matrix)
for i, func in enumerate(funcs):
    if func == "NumMatrix":
        print("initialized NumMatrix class and given matrix is: ")
        for i in range(len(matrix)):
            print(matrix[i])     
    if func == "sumRegion":
        print("sum o the given region: ", params[i], "is:", instance.sumRegion(params[i][0], params[i][1], params[i][2], params[i][3]))        
# Output
# [null, 8, 11, 12]